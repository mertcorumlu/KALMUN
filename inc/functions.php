...
function check_login($PDO,$auth,$auth_levels = array()){
    if(!$auth->isLogged()){
        redirect("/login",true);
    ...
    if(!empty($auth_levels)){
        $user_id=$auth->getSessionUID($_COOKIE[$auth->config->cookie_name]);

        try{
            $query = $PDO->query("SELECT `auth` FROM `phpauth_users` WHERE `id`='{$user_id}'");
